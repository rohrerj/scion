...
func (e *executor) SearchAssetsForStatistics(ctx context.Context, params *StatisticsQuery) ([]*DBStat, []*DBStat, error) {
	...
	publishStmt, args := e.buildPublishedBandwidthQuery(params)
	...
	boughtStmt, args := e.buildBoughtBandwidthQuery(params)
...
func (e *executor) buildPublishedBandwidthQuery(params *StatisticsQuery) (string, []any) {
	var args []any
	where := []string{}
	query := []string{
		"SELECT bandwidth, price, starts_at, stops_at FROM Published_Bandwidth",
	}
...
func (e *executor) buildBoughtBandwidthQuery(params *StatisticsQuery) (string, []any) {
	var args []any
	where := []string{}
	query := []string{
		"SELECT bandwidth, price, starts_at, stops_at FROM Bought_Bandwidth",
	}
	where = append(where, "(isd_id=?) AND (as_id=?) AND (stops_at > ?) AND (starts_at <= ?)")
	args = append(args, int64(params.IA.ISD()), int64(params.IA.AS()), params.WindowStart, params.WindowEnd)
	if params.Ingress != nil {
...
func (e *executor) RegisterPublishedBandwidth(ctx context.Context, a *DBAsset) (int64, error) {
	if e.write == nil {
		return 0, serrors.New("No database open")
	}
	var err error
	inst := `INSERT INTO Published_Bandwidth (isd_id, as_id, ingress, egress, bandwidth, starts_at, stops_at, price)
	VALUES(?,?,?,?,?,?,?,?)`
	res, err := e.write.ExecContext(ctx, inst, a.IA.ISD(), a.IA.AS(), a.IfIdIngress, a.IfIdEgress, a.Bandwidth,
...
func (e *executor) RegisterBoughtBandwidth(ctx context.Context, a *DBAsset) (int64, error) {
	if e.write == nil {
		return 0, serrors.New("No database open")
	}
	var err error
	inst := `INSERT INTO Bought_Bandwidth (isd_id, as_id, ingress, egress, bandwidth, starts_at, stops_at, price)
	VALUES(?,?,?,?,?,?,?,?)`
	res, err := e.write.ExecContext(ctx, inst, a.IA.ISD(), a.IA.AS(), a.IfIdIngress, a.IfIdEgress, a.Bandwidth,
		a.StartAt.UTC().Format(time.RFC3339), a.StopsAt.UTC().Format(time.RFC3339), a.Price)
	if err != nil {
