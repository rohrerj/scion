...
func (m *Match) Policies() map[string][]string {
	return m.hops
}
...
func (m *Match) addMatch(c, start, l int) {
	...
	if m.hops == nil {
		m.hops = make(map[string][]string)
	}
	if m.regex.GroupNameFromNumber(c) == "hop" {
		m.hops[string(m.text[start:start+l])] = m.policies
		m.policies = make([]string, 0)
	} else if m.regex.GroupNameFromNumber(c) == "pol" {
		m.policies = append(m.policies, string(m.text[start:start+l]))
	}
...
func (m *Match) removeMatch(c int) {
	fmt.Println("remove match", c)
	if m.regex.GroupNameFromNumber(c) == "pol" && len(m.policies) > 0 {
